...
/* ════════════════════════════════════════════════════════════════════════════
   ТЕМА — светлая / тёмная. По умолчанию светлая. Влияет на UI (data-theme на <html>)
   и на 3D-сцену (window.setSceneTheme из background.js). Сохраняется в localStorage.
   ════════════════════════════════════════════════════════════════════════════ */
function applyTheme(theme) {
  document.documentElement.setAttribute('data-theme', theme);
  if (typeof window.setSceneTheme === 'function') window.setSceneTheme(theme);
  const btn = document.getElementById('theme-toggle');
  if (btn) btn.setAttribute('aria-pressed', theme === 'dark' ? 'true' : 'false');
  try { localStorage.setItem('ddc_theme', theme); } catch (e) {}
}

let initialTheme = 'light';
try { const s = localStorage.getItem('ddc_theme'); if (s === 'dark' || s === 'light') initialTheme = s; } catch (e) {}
...
// Применяем тему сразу и после загрузки (background.js может выставить
// window.setSceneTheme позже — поэтому повторяем на load).
applyTheme(initialTheme);
window.addEventListener('load', () => applyTheme(initialTheme));
...
window.onBoardRects = function (rects) {
  ...
  if (cc) {
    ...
    if (!IS_MOBILE) {
      if (show && rects.monitor && !rects.monitor.behind && rects.monitor.w > 4) {
        const r = rects.monitor;
        // Вписываем форму В ЭКРАН по принципу contain: масштаб = min(по ширине, по высоте),
        // с лёгким внутренним отступом, чтобы форма не упиралась в рамку экрана.
        const pad = 0.92;
        const scaleW = (r.w * pad) / CONTACT_W;
        const scaleH = (r.h * pad) / CONTACT_H;
        const scale = Math.min(scaleW, scaleH);
        cc.style.setProperty('--cx', r.cx + 'px');
        cc.style.setProperty('--cy', r.cy + 'px');
        cc.style.setProperty('--cc-scale', scale);
        cc.style.visibility = '';
      } else if (show) {
...
// «Дизайнерские» размеры контактной карточки (база для вписывания в экран).
const CONTACT_W = 500;
const CONTACT_H = 430;
...
/* ════════════════════════════════════════════════════════════════════════════
   КОНТАКТНАЯ ФОРМА → отправка заявки на бэкенд (сохраняется в БД, видно в админке)
   ════════════════════════════════════════════════════════════════════════════ */
// Адрес бэкенда. Если сайт и бэкенд на одном origin — оставь пустым ('').
// Иначе укажи, например: 'http://localhost:3000' или 'https://api.tvoy-domen.kz'
const DDC_API_BASE = '';
...
(function initContactForm(){
  ...
  const setError = (msg) => {
    btn.textContent = msg;
    btn.style.background = 'linear-gradient(135deg,#a33,#722)';
    setTimeout(()=>{ resetBtn(); }, 2500);
  };
  const sentText = () => btn.dataset.sentText || 'Отправлено ✓';
  ...
  btn.addEventListener('click', async () => {
    ...
    if(!payload.full_name){ setError('Укажите ФИО'); return; }

    origText = btn.dataset._orig || (btn.dataset._orig = (btn.dataset.i18n ? btn.textContent : btn.textContent));
    btn.disabled = true;
    btn.textContent = '…';

    try{
      const r = await fetch(DDC_API_BASE + '/api/leads', {
        method:'POST',
      ...
      if(r.ok){
        ...
        btn.style.background = 'linear-gradient(135deg,#0e7a4a,#0a5a38)';
        // очистить поля
      ...
      // Бэкенд недоступен (например, открыли просто файл без сервера)
